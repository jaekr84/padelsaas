            ...
            <div className="space-y-6">
               <div className="flex items-center justify-between border-b-2 border-slate-950 pb-4">
                  <div className="flex items-center gap-2">
                     ...
                     <h2 className="text-xl font-black uppercase tracking-tighter">Historial de Reservas</h2>
                  </div>
                  <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Últimas {bookings.length} reservas</span>
               </div>
                     ...
                     <div key={booking.id} className="bg-white border border-slate-200 p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:border-slate-950 transition-colors group">
                        <div className="flex items-center gap-6">
                           ...
                           <div>
                              ...
                              <div className="flex items-center gap-4">
                                 <a
                                    href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${booking.court?.center?.address || ""} ${booking.court?.center?.city || ""} ${booking.court?.center?.state || ""}`.trim())}`}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="flex items-center gap-1.5 text-[9px] font-bold text-slate-400 uppercase tracking-widest hover:text-blue-800 transition-colors group/loc"
                                 >
                                    <LucideMapPin className="h-3 w-3 text-blue-800 group-hover/loc:scale-110 transition-transform" />
                                    {booking.court?.center?.address ? `${booking.court?.center?.address}, ` : ""}{booking.court?.center?.city}
                                 </a>
